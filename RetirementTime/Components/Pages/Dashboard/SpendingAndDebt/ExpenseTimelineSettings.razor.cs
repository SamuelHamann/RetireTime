...
public partial class ExpenseTimelineSettings : ComponentBase
{
    ...
    protected override async Task OnAfterRenderAsync(bool firstRender)
    {
        ...
        _otherTimelines = all.Where(e => e.Id != TimelineId && e.TimelineType == RetirementTimelineTypeEnum.Expenses).ToList();
    ...
    private async Task Save()
    {
        ...
        var result = await Mediator.Send(new SaveExpenseTimelineCommand
        {
            ...
            AgeTo        = _model.AgeTo,
            TimelineType = RetirementTimelineTypeEnum.Expenses,
            IsFirstSave  = !_model.IsFullyCreated,
            CloneFromTimelineId = _model.CloneFromId > 0 ? _model.CloneFromId : null,
        });

        if (result.Success)
            Navigation.NavigateTo($"/scenario/{ScenarioId}/spending/timeline/{TimelineId}/living-expenses", forceLoad: false);
        else
        {
            _errorMessage = result.ErrorMessage ?? Localizer["ExpenseTimeline_SaveError"];
            _isSaving = false;
            StateHasChanged();
        }
    }
    ...
    private async Task Delete()
    {
        ...
        if (result.Success)
            Navigation.NavigateTo($"/scenario/{ScenarioId}/spending", forceLoad: false);
        else
        {
            _errorMessage = result.ErrorMessage ?? Localizer["ExpenseTimeline_DeleteError"];
            _isDeleting = false;
            StateHasChanged();
        }
    }

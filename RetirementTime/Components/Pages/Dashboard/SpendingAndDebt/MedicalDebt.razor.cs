...
public partial class MedicalDebt : ComponentBase
{
    ...
    private static readonly long[] DebtTypeIds = [(long)DebtTypeEnum.Medical];
    ...
    {
        var result = await Mediator.Send(new CreateDebtCommand(ScenarioId, (long)DebtTypeEnum.Medical));
        if (result.Success)
            _items.Add(new GenericDebtItemModel
            {
                Id = result.DebtId, Name = "Medical Debt", DebtTypeId = (long)DebtTypeEnum.Medical,
    ...
    private async Task SaveItem(GenericDebtItemModel item)
    {
        ...
        {
            Id = item.Id, Name = "Medical Debt", DebtTypeId = item.DebtTypeId,

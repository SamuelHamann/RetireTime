...
public partial class CreditCards : ComponentBase
{
    ...
    private static readonly long[] DebtTypeIds = [(long)DebtTypeEnum.CreditCard];
    ...
    {
        var result = await Mediator.Send(new CreateDebtCommand(ScenarioId, (long)DebtTypeEnum.CreditCard));
        if (result.Success)
            _items.Add(new GenericDebtItemModel
            {
                Id = result.DebtId, Name = "Credit Card", DebtTypeId = (long)DebtTypeEnum.CreditCard,
    ...
    private async Task SaveItem(GenericDebtItemModel item)
    {
        ...
        {
            Id = item.Id, Name = "Credit Card", DebtTypeId = item.DebtTypeId,
